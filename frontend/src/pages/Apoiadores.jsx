...
export default function Apoiadores() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6"><div><p className="section-eyebrow mb-1">Base politica</p><h1 className="page-title">Apoiadores e liderancas</h1><p className="page-subtitle">Cadastro, territorio, votos estimados e historico.</p></div>{podeEditar && <button type="button" className="btn-primary w-full sm:w-auto" onClick={() => setAberto(!aberto)}>Novo apoiador</button>}</header>
      ...
      <div className="card overflow-x-auto"><table className="gov-table"><thead><tr><th>Nome</th><th>Contato</th><th>RA / Bairro</th><th>Cidade</th><th>Votos</th><th>Influencia</th><th>Status</th></tr></thead><tbody>{carregando ? <tr><td colSpan="7" className="text-center py-10 text-slate-400">Carregando apoiadores...</td></tr> : dados.map(a => <tr key={a.id}><td><strong>{a.nome}</strong><div className="text-xs text-slate-400">{a.observacoes}</div></td><td>{a.whatsapp || a.telefone || a.email || '-'}</td><td>{a.ra || '-'} / {a.bairro || '-'}</td><td>{a.cidade || '-'} {a.uf || ''}</td><td>{a.votos_estimados}</td><td>{a.nivel_influencia}</td><td>{a.status}</td></tr>)}{!carregando && !dados.length && <tr><td colSpan="7" className="text-center py-10 text-slate-400">Nenhum apoiador cadastrado.</td></tr>}</tbody></table></div>
    </div>
  );
}
