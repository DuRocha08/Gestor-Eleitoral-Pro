  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-6">
        {podeEditar && <form onSubmit={criarQuestionario} className="card p-5 lg:col-span-1"><p className="card-title mb-3">Novo questionario</p><label className="label">Titulo</label><input className="input mb-3" value={formQ.titulo} onChange={e => setFormQ({ ...formQ, titulo: e.target.value })} required /><label className="label">Descricao</label><textarea className="input mb-3" value={formQ.descricao} onChange={e => setFormQ({ ...formQ, descricao: e.target.value })} />{formQ.perguntas.map((p, i) => <div key={i} className="grid grid-cols-2 gap-2 mb-2"><input className="input" value={p.texto} onChange={e => alterarPergunta(i, 'texto', e.target.value)} placeholder="Pergunta" /><select className="input" value={p.tipo} onChange={e => alterarPergunta(i, 'tipo', e.target.value)}>{TIPOS.map(t => <option key={t[0]} value={t[0]}>{t[1]}</option>)}</select></div>)}<button type="button" className="btn-secondary mb-3" onClick={() => setFormQ({ ...formQ, perguntas: formQ.perguntas.concat([{ texto: '', tipo: 'texto' }]) })}>Adicionar pergunta</button><button className="btn-primary w-full">Salvar questionario</button></form>}
        ...
        <div className="card p-5 lg:col-span-2">
          ...
          {linkPublico && <div className="alert-warning mb-4"><p className="mb-2">{estat?.aviso_legal}</p><p className="text-xs break-all">Link publico: {linkPublico}</p><a className="btn-secondary mt-3" href={'https://api.whatsapp.com/send?text=' + encodeURIComponent('Participe da consulta popular: ' + linkPublico)} target="_blank" rel="noreferrer">Compartilhar no WhatsApp</a></div>}
